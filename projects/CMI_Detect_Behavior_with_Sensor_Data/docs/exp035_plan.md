...
# 実装方針
- `codes/exp/exp031` を `codes/exp/exp035` にコピーしてからexp035を実装すること
  - test_*.pyはいらない
- 差分は指示されたもの以外最小限にすること 
- `codes/exp/exp031` からのインポートは禁止
- `codes/exp/exp035` 以外の `codes/exp/` 配下の他のexp directory配下のファイルの編集は禁止
- config.pyの以下の項目は必ず更新して
  - `ExperimentConfig.description`
  - `ExperimentConfig.tags`
  - `LoggingConfig.wandb_tags`
# 新規実装アイデア
- codes/exp/exp027/model.py を参照して、 headの手前にbertを挟む実装をして