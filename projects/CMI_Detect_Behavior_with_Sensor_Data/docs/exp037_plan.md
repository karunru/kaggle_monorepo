...
# 実装方針
- `codes/exp/exp036` を `codes/exp/exp037` にコピーしてからexp037を実装すること
  - test_*.pyはいらない
- 差分は指示されたもの以外最小限にすること 
- `codes/exp/exp036` からのインポートは禁止
- `codes/exp/exp037` 以外の `codes/exp/` 配下の他のexp directory配下のファイルの編集は禁止
- config.pyの以下の項目は必ず更新して
  - `ExperimentConfig.description`
  - `ExperimentConfig.tags`
  - `LoggingConfig.wandb_tags`
# 新規実装アイデア
- docs/chatgpt_orient_constraints.md を参照して、1) 補助タスク：orient head を追加 を実装して