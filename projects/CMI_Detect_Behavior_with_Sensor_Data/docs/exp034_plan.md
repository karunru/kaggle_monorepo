...
# 実装方針
- `codes/exp/exp031` を `codes/exp/exp034` にコピーしてからexp034を実装すること
  - test_*.pyはいらない
- 差分は指示されたもの以外最小限にすること 
- `codes/exp/exp031` からのインポートは禁止
- `codes/exp/exp034` 以外の `codes/exp/` 配下の他のexp directory配下のファイルの編集は禁止
- config.pyの以下の項目は必ず更新して
  - `ExperimentConfig.description`
  - `ExperimentConfig.tags`
  - `LoggingConfig.wandb_tags`
# 新規実装アイデア
- docs/chatgpt_Fourier_Neural_Operator.mdを参照して、実装プランBのGRU+Attentionの置換：AFNOミキサを実装して