...
# 実装方針
- `codes/exp/exp036` を `codes/exp/exp045` にコピーしてからexp045を実装すること
  - test_*.pyはいらない
- 差分は指示されたもの以外最小限にすること 
- `codes/exp/exp036` からのインポートは禁止
- `codes/exp/exp045` 以外の `codes/exp/` 配下の他のexp directory配下のファイルの編集は禁止
...
- trainのときはphaseがGestureだけの時系列に切り出して、validation, inferenceのときはTransitionも含むすべての時系列でforwardするようにして