...
from models import FreezeUnfreezeCallback
...
def main():
    ...
    wandb_logger = WandbLogger(
        project="Harmful_Brain_Activity_Classification",
        name=f"exp_{exp_num}",
    )
    ...
    df = pd.read_csv(Path(config.data_path) / "non_overlap_train.csv")
    target_cols = df.columns[-6:].tolist()
    if config.label_smoothing:
        df[target_cols] = df[target_cols] + (1 / 6)
        df[target_cols] = df[target_cols] / df[target_cols].to_numpy().sum(axis=1, keepdims=True)
    oof_pred_cols = [f"oof_{target_col}" for target_col in target_cols]
    df[oof_pred_cols] = -1
    num_features = (
        config.model.params.emb_size
        if config.model.name == "ConvTranModule"
        else create_model(
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        if config.filter_over_10:
            train_df = train_df[train_df["max_votes"] >= 10]
        if config.update_under_10_labels.do:
            oof_pred_path = str(config.pred_path).replace(str(exp_num), config.update_under_10_labels.oof_exp)
            oof_pred_df = pd.read_csv(Path(oof_pred_path) / "oof_pred.csv")
            low_quality_eeg_ids = train_df.loc[train_df["max_votes"] < 10, "eeg_id"]
            oof_pred_df = oof_pred_df.loc[oof_pred_df["eeg_id"].isin(low_quality_eeg_ids), :]

            low_quality_target = train_df.loc[train_df["max_votes"] < 10, target_cols].to_numpy()
            oof_pred_labels = oof_pred_df.loc[oof_pred_df["eeg_id"].isin(low_quality_eeg_ids), oof_pred_cols].to_numpy()

            train_df.loc[train_df["max_votes"] < 10, target_cols] = (
                low_quality_target * config.update_under_10_labels.alpha
                + oof_pred_labels * (1 - config.update_under_10_labels.alpha)
            )
        ...
        if config.scheduler.name == "OneCycleLR":
            config.scheduler.params.steps_per_epoch = int(len(train_df) // config.train_loader.batch_size)
        model = eval(config.model.name)(config)
        if config.load_weight.do:
            weight_paths = sorted(
                list(
                    Path(str(config.weight_path).replace(str(exp_num), config.load_weight.exp)).glob(
                        f"fold_{fold}_best_loss*"
                    )
                )
            )
            last_weight_path = weight_paths[-2] if len(weight_paths) >= 2 else weight_paths[0]
        ...
        freeze_epoch = FreezeUnfreezeCallback(freeze_epoch=config.load_weight.freeze_epoch)
    ...
    wandb_logger.experiment.config.update(
        {
            "oof_score": kaggle_kl_div_score(
                ...
                submission=df.loc[df[oof_pred_cols[0]] >= 0, oof_pred_cols + ["id"]].rename(
                    columns=dict(zip(oof_pred_cols, target_cols))
                ),
                row_id_column_name="id",
            )
        }
