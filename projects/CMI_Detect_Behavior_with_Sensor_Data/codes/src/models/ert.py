...
class ExtremelyRandomizedTrees(BaseModel):
    ...
    def predict(self, model: ERTModel, features: pd.DataFrame | np.ndarray) -> np.ndarray:
        ...
        numerical_cols = [
            col for col in features.columns if col not in features.select_dtypes(include="category").columns
        ]
        for col in numerical_cols:
