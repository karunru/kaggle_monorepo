...
class BaseGateway:
    def __init__(
        ...
        self.client = relay.Client("inference_server" if IS_RERUN else "localhost")
    ...
    def validate_prediction_batch(
        self, prediction_batch: Any, row_ids: pl.DataFrame | pl.Series | pd.DataFrame | pd.Series
    ) -> None:
    ...
    def _standardize_and_validate_paths(self, input_paths: list[str | pathlib.Path]) -> tuple[list[str], list[str]]:
        ...
        if (input_paths and not self.file_share_dir) or type(self.file_share_dir) not in (str, pathlib.Path):
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, f"Invalid `file_share_dir`: {self.file_share_dir}"
            )

        for path in input_paths:
            if os.pardir in str(path):
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, f"Send files path contains {os.pardir}: {path}"
                )
            ...
            if type(path) not in (pathlib.Path, str):
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, "All paths must be of type str or pathlib.Path"
                )
            if not os.path.exists(path):
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, f"Input path {path} does not exist"
                )
    ...
    def share_files(
        ...
        if self.file_share_dir and not self._shared_a_file:
            if os.path.exists(self.file_share_dir) and (
                not os.path.isdir(self.file_share_dir) or len(os.listdir(self.file_share_dir)) > 0
            ):
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, "`file_share_dir` must be an empty directory."
                )
        ...
        if not input_paths:
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, "share_files requires at least one input path"
            )
    ...
    def write_submission(self, predictions, row_ids: list[pl.Series | pl.DataFrame | pd.Series | pd.DataFrame]) -> None:
        ...
        if isinstance(predictions, list):
            if isinstance(predictions[0], pd.DataFrame):
                ...
                try:
                    ...
                    raise GatewayRuntimeError(
                        GatewayRuntimeErrorType.INVALID_SUBMISSION, "Inconsistent prediction column counts"
                    )
            else:
    ...
    def handle_server_error(self, exception: Exception, endpoint: str) -> None:
        ...
        if isinstance(exception, gaierror) or (
            isinstance(exception, RuntimeError) and "Failed to connect to server after waiting" in exception_str
        ):
        ...
        if f"No listener for {endpoint} was registered" in exception_str:
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.SERVER_MISSING_ENDPOINT, f"Server did not register a listener for {endpoint}"
            ) from None
        ...
        if isinstance(exception, relay.GRPCDeadlineError):
