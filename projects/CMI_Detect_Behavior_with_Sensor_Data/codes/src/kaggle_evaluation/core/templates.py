...
from . import base_gateway, relay

_initial_import_time = time.time()
_issued_startup_time_warning = False


class Gateway(base_gateway.BaseGateway, abc.ABC):
    """
    ...
    def run(self) -> None:
        ...
        try:
            ...
            error = base_gateway.GatewayRuntimeError(
                base_gateway.GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION, error_str
            )
...
class InferenceServer(abc.ABC):
    ...
    def __init__(self, *endpoint_listeners: Callable):
        self.server = relay.define_server(*endpoint_listeners)
        ...
        self._startup_limit_seconds = relay.STARTUP_LIMIT_SECONDS
    ...
    def run_local_gateway(
        self, data_paths: tuple[str] | None = None, file_share_dir: str = None, *args, **kwargs
    ) -> None:
