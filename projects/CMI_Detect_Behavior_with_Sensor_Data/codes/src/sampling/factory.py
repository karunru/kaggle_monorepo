...
def smote(x_trn: np.ndarray, y_trn: np.ndarray, config: dict) -> tuple[np.ndarray, np.ndarray]:
...
def random_under_sample(x_trn: np.ndarray, y_trn: np.ndarray, config: dict) -> tuple[np.ndarray, np.ndarray]:
    params = config["model"]["sampling"]["params"]
...
def random_under_sample_and_smote(x_trn: np.ndarray, y_trn: np.ndarray, config: dict) -> tuple[np.ndarray, np.ndarray]:
    sampled_x, sampled_y = random_under_sample(x_trn, y_trn, config)
    sampled_x, sampled_y = smote(sampled_x, sampled_y, config)
    return sampled_x, sampled_y


def get_sampling(x_trn: AoD, y_trn: AoS, config: dict) -> tuple[AoD, AoS]:
    ...
    X, y = func(
        x_trn.to_pandas() if hasattr(x_trn, "to_pandas") else x_trn,
        y_trn,
        config,
    )

    return (
        pl.from_pandas(X) if isinstance(x_trn, pl.DataFrame) else X,
        y,
    )
