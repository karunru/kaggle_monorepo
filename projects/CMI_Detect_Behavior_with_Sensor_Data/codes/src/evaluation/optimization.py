...
class GroupWiseOptimizer:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, group: np.ndarray):
        self.rounders = {
            gp: OptimizedRounder(n_overall=self.n_overall, n_classwise=self.n_classwise) for gp in np.unique(group)
        }
...
class OptimizedRounderNotScaled:
    def __init__(self):
        self.coef_ = 0
