...
def eval_with_truncated_data(
    ...
    eval_result: dict[str, list[float] | float] = {}
    trials: list[float] = []
...
def truncated_cv_with_adjustment_of_distribution(
    ...
    for i in range(n_trials):
        ...
        dice = np.random.choice(over_fifth_assess_idx, size=n_over_five_assess, replace=False)
        over_fifth_assess_dice.append(dice)

    over_fifth_assess = np.asarray(over_fifth_assess_dice).T

    assess = np.vstack([first_assess, second_assess, third_assess, fourth_assess, over_fifth_assess])
