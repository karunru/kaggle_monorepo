...
class CMISqueezeformer(pl.LightningModule):
    ...
    def _compute_uncertainty_weighted_loss(self, multiclass_loss, binary_loss, nine_class_loss, kl_loss):
        """不確かさベース自動重み付き損失の計算."""
        if not hasattr(self, "loss_s_params"):
            raise RuntimeError("loss_s_params not initialized. Call _setup_learnable_weights first.")
        ...
        terms.append(torch.exp(-s_mc) * multiclass_loss + s_mc)

        # バイナリ損失
        s_bin = self.loss_s_params["binary"].clamp(*clamp_range)
        terms.append(torch.exp(-s_bin) * binary_loss + s_bin)
    ...
    def _compute_direct_weighted_loss(self, multiclass_loss, binary_loss, nine_class_loss, kl_loss):
        """直接的な学習可能重み付き損失の計算."""
        if not hasattr(self, "alpha_raw"):
            raise RuntimeError("Direct weighting parameters not initialized. Call _setup_learnable_weights first.")

        # alpha = sigmoid(alpha_raw)
        alpha = torch.sigmoid(self.alpha_raw)

        # w9 = exp(w9_raw), wkl = exp(wkl_raw)
    ...
    def _log_learnable_weights(self):
        """学習された重みをログ出力."""
        aw_type = self.loss_config.get("auto_weighting", "none")

        if aw_type == "uncertainty" and hasattr(self, "loss_s_params"):
            clamp_range = self.loss_config.get("auto_weight_clamp", (-10.0, 10.0))
            for name, param in self.loss_s_params.items():
                s_val = param.clamp(*clamp_range)
                weight = torch.exp(-s_val)
                self.log(f"weight_{name}", weight.detach())
                self.log(f"s_{name}", s_val.detach())

        elif aw_type == "direct" and hasattr(self, "alpha_raw"):
            alpha = torch.sigmoid(self.alpha_raw)
            w9 = torch.exp(self.w9_raw)
            wkl = torch.exp(self.wkl_raw)

            self.log("alpha", alpha.detach())
            self.log("weight_nine_class", w9.detach())
            self.log("weight_kl", wkl.detach())
            self.log("weight_binary", (1 - alpha).detach())  # binary weight = 1 - alpha
    ...
    def training_step(self, batch: dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        aw_type = self.loss_config.get("auto_weighting", "none")
        if aw_type == "uncertainty":
            total_loss = self._compute_uncertainty_weighted_loss(multiclass_loss, binary_loss, nine_class_loss, kl_loss)
        elif aw_type == "direct":
            total_loss = self._compute_direct_weighted_loss(multiclass_loss, binary_loss, nine_class_loss, kl_loss)
        else:
            # 従来の固定重み損失
            total_loss = (
    ...
    def validation_step(self, batch: dict[str, torch.Tensor], batch_idx: int):
        ...
        # 学習可能重みのログ出力（validation時は頻度を下げる）
        if batch_idx == 0:  # 最初のバッチでのみログ出力
            self._log_learnable_weights()
    ...
    def compute_kl_loss(self, multiclass_logits: torch.Tensor, nine_class_logits: torch.Tensor) -> torch.Tensor:
        ...
        # nn.KLDivLossはinputにlog_p、targetにpを期待
