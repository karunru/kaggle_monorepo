...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_kl_divergence_loss", description="実験名")
    description: str = Field(
        default="KL divergence loss between multiclass and nine-class distributions for consistency",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["kl_divergence", "multiclass_consistency", "hierarchical_loss", "squeezeformer", "pytorch_lightning"],
        description="実験タグ",
    )
...
class LossConfig(BaseModel):
    ...
    auto_weighting: Literal["none", "uncertainty", "direct"] = Field(
        default="uncertainty", description="自動重み付け方式"
    )
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(
        default=[EXP_NUM, "bert", "attention", "transformer", "squeezeformer"], description="WandBタグ"
    )
