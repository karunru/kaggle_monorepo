...
class ExperimentConfig(BaseModel):
    ...
    description: str = Field(
        default="EMA (Exponential Moving Average) integration from ema-pytorch",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
...
class EMAConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="EMA使用の有効/無効")
...
class TrainingConfig(BaseModel):
    ...
    learning_rate: float = Field(default=6e-4, description="学習率")
