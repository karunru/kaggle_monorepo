...
class IMUOnlyLSTM(nn.Module):
    ...
    def __init__(
        ...
        weight_decay=1e-4,
        demographics_dim=0,
        statistical_dim=0,  # 統計量特徴量の次元を追加
        dropout=0.1,
        ...
        self.imu_block1 = ResidualSECNNBlock(
            imu_dim, cnn_block1_out, cnn_block1_kernel, dropout=cnn_dropout, weight_decay=weight_decay
        )
        self.imu_block2 = ResidualSECNNBlock(
            cnn_block1_out, cnn_block2_out, cnn_block2_kernel, dropout=cnn_dropout, weight_decay=weight_decay
        )
        ...
        # Classification heads (IMU特徴量 + Demographics特徴量 + 統計量特徴量を結合)
        classification_input_dim = dense2_dim + self.demographics_dim + self.statistical_dim
    ...
    def forward(self, x, demographics_embedding=None, statistical_features=None):
        """
        前向き計算.
        ...
        Args:
            ...
            statistical_features: 統計量特徴量 [batch, statistical_dim] (オプション)
        ...
        # Demographics特徴量と統計量特徴量との結合
        features_to_combine = [imu_features]

        if demographics_embedding is not None and self.demographics_dim > 0:
            features_to_combine.append(demographics_embedding)

        if statistical_features is not None:
            features_to_combine.append(statistical_features)

        combined_features = torch.cat(features_to_combine, dim=-1) if len(features_to_combine) > 1 else imu_features
...
class CMISqueezeformer(pl.LightningModule):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        statistical_dim: int = 182,  # 統計量特徴量の次元（20 features * 9 stats + 2 sequence features）
        ...
        self.model = IMUOnlyLSTM(
            ...
            statistical_dim=statistical_dim,  # 統計量特徴量の次元を追加
            dropout=dropout,
    ...
    def forward(
        ...
        Args:
            ...
            statistical_features: 統計量特徴量 [batch, statistical_dim] （オプショナル）
        ...
        return self.model(imu, demographics_embedding, statistical_features)
    ...
    def training_step(self, batch: dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        statistical_features = batch.get("statistical_features", None)  # 統計量特徴量（オプショナル）

        # 前向き計算
        multiclass_logits, binary_logits, nine_class_logits = self(
            imu, attention_mask, demographics, statistical_features
        )
...
if __name__ == "__main__":
    ...
    imu_model = IMUOnlyLSTM(imu_dim=20, n_classes=18, demographics_dim=16, model_config=None)
