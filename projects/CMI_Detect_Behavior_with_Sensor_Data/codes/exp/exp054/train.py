...
def train_single_fold(config: Config, fold: int, logger, wandb_logger) -> dict:
    ...
    model = CMISqueezeformer(
        ...
        model_config=config.model,  # 追加
        statistical_dim=182,  # 統計量特徴量の次元を明示的に指定
    ...
    if hasattr(trainer.checkpoint_callback, "best_model_path"):
        ...
        if best_model_path:
            ...
            # Load model with explicit statistical_dim parameter
            model = CMISqueezeformer.load_from_checkpoint(
                best_model_path,
                statistical_dim=182,  # 統計量特徴量の次元を明示的に指定
            )
