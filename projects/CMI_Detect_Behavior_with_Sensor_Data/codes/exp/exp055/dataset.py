...
def add_angular_acceleration_features_pl(df: pl.DataFrame) -> pl.DataFrame:
    """
    Add angular acceleration features using Polars.

    Args:
        df: Polars DataFrame with angular velocity columns

    Returns:
        DataFrame with added angular acceleration features
    """
    # Calculate angular acceleration (derivative of angular velocity)
    return df.with_columns(
        [
            pl.col("angular_vel_x").diff().over("sequence_id").fill_null(0).alias("angular_vel_x_acc"),
            pl.col("angular_vel_y").diff().over("sequence_id").fill_null(0).alias("angular_vel_y_acc"),
            pl.col("angular_vel_z").diff().over("sequence_id").fill_null(0).alias("angular_vel_z_acc"),
        ]
    )


def add_angular_jerk_snap_features_pl(df: pl.DataFrame) -> pl.DataFrame:
    """
    Add angular jerk and snap features using Polars.

    Args:
        df: Polars DataFrame with angular acceleration columns

    Returns:
        DataFrame with added angular jerk and snap features
    """
    # Angular jerk (derivative of angular acceleration)
    df = df.with_columns(
        [
            pl.col("angular_vel_x_acc").diff().over("sequence_id").fill_null(0).alias("angular_jerk_x"),
            pl.col("angular_vel_y_acc").diff().over("sequence_id").fill_null(0).alias("angular_jerk_y"),
            pl.col("angular_vel_z_acc").diff().over("sequence_id").fill_null(0).alias("angular_jerk_z"),
        ]
    )

    # Angular snap (derivative of angular jerk)
    df = df.with_columns(
        [
            pl.col("angular_jerk_x").diff().over("sequence_id").fill_null(0).alias("angular_snap_x"),
            pl.col("angular_jerk_y").diff().over("sequence_id").fill_null(0).alias("angular_snap_y"),
            pl.col("angular_jerk_z").diff().over("sequence_id").fill_null(0).alias("angular_snap_z"),
        ]
    )
...
def add_acc_rot_ratio_feature_pl(df: pl.DataFrame) -> pl.DataFrame:
    """
    Add acceleration to rotation ratio feature using Polars.

    Args:
        df: Polars DataFrame with acc_mag and rot_angle columns

    Returns:
        DataFrame with added acc_rot_ratio feature
    """
    return df.with_columns([(pl.col("acc_mag") / (pl.col("rot_angle") + 1e-6)).alias("acc_rot_ratio")])


def add_rolling_statistics_features_pl(df: pl.DataFrame, window_size: int) -> pl.DataFrame:
    """
    Add rolling window statistics features using Polars.
    ...
    for col in acc_cols:
        df = df.with_columns(
            [
                pl.col(col).rolling_mean(window_size).over("sequence_id").alias(f"{col}_rolling_mean_{window_size}"),
                pl.col(col).rolling_std(window_size).over("sequence_id").alias(f"{col}_rolling_std_{window_size}"),
                (pl.col(col).rolling_std(window_size).over("sequence_id").sqrt()).alias(
                    f"{col}_rolling_std_sqrt_{window_size}"
                ),
            ]
        )

    # Linear acceleration rolling statistics
    linear_acc_cols = ["linear_acc_x", "linear_acc_y", "linear_acc_z"]
    for col in linear_acc_cols:
        df = df.with_columns(
            [
                pl.col(col)
                .rolling_median(window_size)
                .over("sequence_id")
                .alias(f"{col}_rolling_median_{window_size}"),
                pl.col(col).rolling_max(window_size).over("sequence_id").alias(f"{col}_rolling_max_{window_size}"),
                pl.col(col).rolling_min(window_size).over("sequence_id").alias(f"{col}_rolling_min_{window_size}"),
            ]
        )

    # Rotation rolling statistics
...
def feature_engineering_jiazhuang_exp055(df: pl.DataFrame) -> tuple[pl.DataFrame, list[str]]:
    """
    Apply jiazhuang notebook feature engineering with exp055 additions using Polars.

    Args:
        df: Polars DataFrame with IMU data

    Returns:
        Tuple of (DataFrame with features, list of feature names)
    """
    # Convert to pandas for existing feature engineering
    df_pd = df.to_pandas()
    df_pd, base_imu_features = feature_engineering_jiazhuang(df_pd)

    # Convert back to Polars
    df_pl = pl.from_pandas(df_pd)

    # Add new features using Polars
    df_pl = add_angular_acceleration_features_pl(df_pl)
    df_pl = add_angular_jerk_snap_features_pl(df_pl)
    df_pl = add_acc_rot_ratio_feature_pl(df_pl)

    # Add rolling statistics for different window sizes
    for window_size in [3, 5, 10]:
        df_pl = add_rolling_statistics_features_pl(df_pl, window_size)

    # Define all IMU feature names including new ones
    new_features = [
        ...
        "angular_jerk_y",
        "angular_jerk_z",
        # Angular snap features
        "angular_snap_x",
        "angular_snap_y",
        "angular_snap_z",
        # Acceleration-rotation ratio
        "acc_rot_ratio",
    ]
    ...
    for window_size in [3, 5, 10]:
        ...
        # Linear acceleration rolling stats
        for col in ["linear_acc_x", "linear_acc_y", "linear_acc_z"]:
            rolling_features.extend(
                [
                    f"{col}_rolling_median_{window_size}",
                    f"{col}_rolling_max_{window_size}",
                    f"{col}_rolling_min_{window_size}",
                ]
            )
        # Rotation rolling stats
    ...
    all_imu_features = base_imu_features + new_features + rolling_features

    # Fill missing values and convert to float32
    df_pl = df_pl.with_columns([pl.col(col).fill_null(0).cast(pl.Float32) for col in all_imu_features])

    return df_pl, all_imu_features
...
class IMUDataset(Dataset):
    ...
    def __init__(
        ...
        # IMU列の定義（exp055 extended features: base 19 + exp055 additions）
        self.imu_cols = [
            ...
            # Angular distance (1)
            "angular_distance",
            # exp055 new features - Angular acceleration (3)
            "angular_vel_x_acc",
            "angular_vel_y_acc",
            "angular_vel_z_acc",
            # exp055 new features - Angular jerk (3)
            "angular_jerk_x",
            "angular_jerk_y",
            "angular_jerk_z",
            # exp055 new features - Angular snap (3)
            "angular_snap_x",
            "angular_snap_y",
            "angular_snap_z",
            # exp055 new features - Acc-rot ratio (1)
            "acc_rot_ratio",
        ]

        # Add rolling statistics features
        for window_size in [3, 5, 10]:
            # Acceleration rolling statistics (9 per window)
            for col in ["acc_x", "acc_y", "acc_z"]:
                for stat in ["mean", "std", "std_sqrt"]:
                    self.imu_cols.append(f"{col}_rolling_{stat}_{window_size}")

            # Linear acceleration rolling statistics (9 per window)
            for col in ["linear_acc_x", "linear_acc_y", "linear_acc_z"]:
                for stat in ["median", "max", "min"]:
                    self.imu_cols.append(f"{col}_rolling_{stat}_{window_size}")

            # Rotation rolling statistics (12 per window)
            for col in ["rot_w", "rot_x", "rot_y", "rot_z"]:
                for stat in ["mean", "std", "std_sqrt"]:
                    self.imu_cols.append(f"{col}_rolling_{stat}_{window_size}")

        rolling_count = len(self.imu_cols) - 29
        total_count = len(self.imu_cols)
        print(f"Total IMU features: {total_count} (19 base + 10 exp055 physics + {rolling_count} rolling statistics)")
    ...
    def _add_physics_features(self, df: pl.DataFrame) -> pl.DataFrame:
        """
        jiazhuang notebook compatible: 19 IMU physical features + exp055 extensions.

        Args:
            df: 元のIMUデータを含むDataFrame

        Returns:
            物理特徴量が追加されたDataFrame (19 + exp055 features total)
        """
        print("Computing jiazhuang notebook compatible IMU features (19 features) + exp055 extensions...")
        ...
        # 全ての物理特徴量を一度に計算して結合 (jiazhuang notebook compatible + exp055 extensions)
        df_with_physics = (
            ...
            .with_columns(
                [
                    ...
                    # exp055 new features - Angular acceleration
                    pl.col("angular_vel_x").diff().over("sequence_id").fill_null(0.0).alias("angular_vel_x_acc"),
                    pl.col("angular_vel_y").diff().over("sequence_id").fill_null(0.0).alias("angular_vel_y_acc"),
                    pl.col("angular_vel_z").diff().over("sequence_id").fill_null(0.0).alias("angular_vel_z_acc"),
                    # exp055 new features - acc_rot_ratio
                    (
                        (pl.col("acc_x") ** 2 + pl.col("acc_y") ** 2 + pl.col("acc_z") ** 2).sqrt()
                        / (2 * pl.col("rot_w").clip(-1, 1).arccos() + 1e-6)
                    ).alias("acc_rot_ratio"),
                ]
            )
            # Add angular jerk features
            .with_columns(
                [
                    pl.col("angular_vel_x_acc").diff().over("sequence_id").fill_null(0.0).alias("angular_jerk_x"),
                    pl.col("angular_vel_y_acc").diff().over("sequence_id").fill_null(0.0).alias("angular_jerk_y"),
                    pl.col("angular_vel_z_acc").diff().over("sequence_id").fill_null(0.0).alias("angular_jerk_z"),
                ]
            )
            # Add angular snap features
            .with_columns(
                [
                    pl.col("angular_jerk_x").diff().over("sequence_id").fill_null(0.0).alias("angular_snap_x"),
                    pl.col("angular_jerk_y").diff().over("sequence_id").fill_null(0.0).alias("angular_snap_y"),
                    pl.col("angular_jerk_z").diff().over("sequence_id").fill_null(0.0).alias("angular_snap_z"),
                ]
            )
            .collect()
        )
        ...
        # Add rolling statistics features using the existing df_with_physics
        df_pl = df_with_physics.lazy()
            ...
            for col in ["acc_x", "acc_y", "acc_z"]:
                df_pl = df_pl.with_columns(
                    [
                        ...
                        .over("sequence_id")
                        .alias(f"{col}_rolling_mean_{window_size}"),
                        pl.col(col)
                        .rolling_std(window_size)
                        .over("sequence_id")
                        .alias(f"{col}_rolling_std_{window_size}"),
                        (pl.col(col).rolling_std(window_size).over("sequence_id").sqrt()).alias(
                            f"{col}_rolling_std_sqrt_{window_size}"
                        ),
                    ]
            ...
            for col in ["linear_acc_x", "linear_acc_y", "linear_acc_z"]:
                df_pl = df_pl.with_columns(
                    [
                        ...
                        .over("sequence_id")
                        .alias(f"{col}_rolling_max_{window_size}"),
                        pl.col(col)
                        .rolling_min(window_size)
                        .over("sequence_id")
                        .alias(f"{col}_rolling_min_{window_size}"),
                    ]
                )
        ...
        df_with_physics_and_rolling = df_pl.collect()
        print(f"Added rolling statistics features. Final DataFrame shape: {df_with_physics_and_rolling.shape}")

        # Verify 19 base IMU features are available
        base_imu_features = [
            "acc_x",
            "acc_y",
            "acc_z",
        ...
        available_base_features = [col for col in base_imu_features if col in df_with_physics_and_rolling.columns]
        base_count = len(available_base_features)
        print(f"Available base IMU features ({base_count}/19): {base_count} features")

        if len(available_base_features) != 19:
            missing_features = [col for col in base_imu_features if col not in df_with_physics_and_rolling.columns]
            print(f"Warning: Missing base IMU features: {missing_features}")

        # Count exp055 additional features
        exp055_features = [
            "angular_vel_x_acc",
            "angular_vel_y_acc",
            "angular_vel_z_acc",
        ...
        rolling_feature_count = 0
        for window_size in [3, 5, 10]:
            for col in ["acc_x", "acc_y", "acc_z"]:
                for stat in ["mean", "std", "std_sqrt"]:
                    rolling_feature_count += 1
            for col in ["linear_acc_x", "linear_acc_y", "linear_acc_z"]:
                for stat in ["median", "max", "min"]:
                    rolling_feature_count += 1
            for col in ["rot_w", "rot_x", "rot_y", "rot_z"]:
        ...
        available_exp055_features = [col for col in exp055_features if col in df_with_physics_and_rolling.columns]
        feature_summary = f"{len(available_exp055_features)}/10 physical + {rolling_feature_count} rolling statistics"
        print(f"Added exp055 features: {feature_summary}")
        ...
        if self.use_demographics and self.demographics_config.hn_enabled:
            ...
            df_with_hn = compute_hn_features(
                df_with_physics_and_rolling.lazy(), self.demographics_data, hn_config
            ).collect()
            print(f"Added Human Normalization features. DataFrame shape: {df_with_hn.shape}")
            return df_with_hn

        return df_with_physics_and_rolling
