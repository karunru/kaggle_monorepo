...
class IMUDataset(Dataset):
    ...
    def __init__(
        ...
        # 体位ラベルのマッピング作成
        self._create_orientation_mappings()
    ...
    def _create_orientation_mappings(self):
        """体位ラベルのマッピングを作成."""
        # Orientation mapping (4 classes)
        self.orientation_to_id = {
            "Seated Straight": 0,
            "Lie on Back": 1,
            "Lie on Side - Non Dominant": 2,
            "Seated Lean Non Dom - FACE DOWN": 3,
        }
        self.id_to_orientation = {idx: orientation for orientation, idx in self.orientation_to_id.items()}
        print(f"Created orientation mapping: {self.orientation_to_id}")
            ...
            .agg(
                [
                    ...
                    *[pl.col(col) for col in available_cols],
                    ...
                    pl.col("orientation").first().alias("orientation"),
                ]
            )
            .sort("sequence_id")
        )
    ...
    def _normalize_sequences_parallel_with_mask(self, processed_data: pl.DataFrame) -> dict:
        ...
        def process_sequence_batch(batch_data: list[tuple]) -> list[tuple]:
            ...
            for row_data in batch_data:
                seq_id = row_data[0]
                # IMUデータを取得（各列のリスト）
                imu_lists = row_data[1:-2]  # 最後の2つはgestureとorientation
                gesture = row_data[-2]
                orientation = row_data[-1]
                ...
                orientation_label = self.orientation_to_id[orientation]
        ...
        for i in range(0, len(processed_data), batch_size):
            ...
            for j in range(len(batch)):
                ...
                # gestureとorientationを追加
                row_data.append(batch_dict["gesture"][j])
                row_data.append(batch_dict["orientation"][j])
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        result = {
            ...
            "orientation_label": torch.tensor(data["orientation_label"], dtype=torch.long),
            "sequence_id": seq_id,
