...
class MulticlassSoftF1ACLS(nn.Module):
    """マルチクラス分類用のSoftF1Loss with ACLS実装（Macro F1ベース + ACLS正則化）."""
    ...
    def __init__(
        ...
        Args:
            ...
            pos_lambda: 正サンプル正則化重み
            neg_lambda: 負サンプル正則化重み
            alpha: 正則化項の全体重み
            margin: 距離計算のマージン
        """
        super().__init__()
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            inputs: 予測ロジット [batch, num_classes] または確率値
            targets: ターゲットラベル [batch] (クラスID)

        Returns:
            SoftF1ACLS損失 (SoftF1Loss + ACLS正則化)
        """
        # Handle both logits and probabilities
        if inputs.requires_grad and inputs.max() > 1.0:
        ...
        # SoftF1Loss計算
        soft_f1_loss = self._compute_soft_f1_loss(probs, targets)

        # ACLS正則化項の計算
        if self.alpha > 0:
            reg_loss = self._compute_acls_regularization(logits, targets)
            total_loss = soft_f1_loss + self.alpha * reg_loss
        else:
            total_loss = soft_f1_loss

        return total_loss

    def _compute_soft_f1_loss(self, probs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """SoftF1Lossの計算."""
        # one-hotエンコーディング
    ...
    def _compute_acls_regularization(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """ACLS正則化項の計算."""
        ...
        if pos_mask.any():
            # Positive samples: correctly predicted
            pos_logits = logits[pos_mask]  # [num_pos, num_classes]
            pos_targets = targets[pos_mask]  # [num_pos]

            # Distance calculation for positive samples
            pos_target_logits = pos_logits.gather(1, pos_targets.unsqueeze(1)).squeeze(1)  # [num_pos]
            max_logits = pos_logits.max(dim=-1)[0]  # [num_pos]

            pos_distances = F.relu(max_logits - pos_target_logits - self.margin)
            pos_reg = pos_distances.mean()
        ...
        if neg_mask.any():
            # Negative samples: incorrectly predicted
            neg_logits = logits[neg_mask]  # [num_neg, num_classes]
            neg_targets = targets[neg_mask]  # [num_neg]

            # Distance calculation for negative samples
            neg_target_logits = neg_logits.gather(1, neg_targets.unsqueeze(1)).squeeze(1)  # [num_neg]
            max_logits = neg_logits.max(dim=-1)[0]  # [num_neg]

            neg_distances = F.relu(max_logits - neg_target_logits - self.margin)
            neg_reg = neg_distances.mean()
...
class BinarySoftF1ACLS(nn.Module):
    """バイナリ分類用のSoftF1Loss with ACLS実装."""
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            inputs: 予測ロジット [batch] または確率値 (sigmoid後の値)
        ...
        if inputs.max() > 1.0 or inputs.min() < 0.0:
            ...
            # Assume probabilities
            probs = inputs
            logits = torch.log(probs / (1 - probs + self.eps) + self.eps)

        # SoftF1Loss計算
        soft_f1_loss = self._compute_binary_soft_f1_loss(probs, targets)

        # ACLS正則化項の計算
        if self.alpha > 0:
            # Convert to binary logits for ACLS
            binary_logits = torch.stack([-logits, logits], dim=-1)
            reg_loss = self._compute_acls_regularization(binary_logits, targets.long())
            total_loss = soft_f1_loss + self.alpha * reg_loss
        else:
            total_loss = soft_f1_loss
    ...
    def _compute_binary_soft_f1_loss(self, probs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """Binary SoftF1Lossの計算."""
        # True Positives, False Positives, False Negatives
    ...
    def _compute_acls_regularization(self, binary_logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """Binary ACLS正則化項の計算."""
        batch_size = binary_logits.size(0)

        # Get predicted class
        pred_class = binary_logits.argmax(dim=-1)  # [batch_size]
        ...
        if pos_mask.any():
            # Positive samples
            pos_logits = binary_logits[pos_mask]  # [num_pos, 2]
            pos_targets = targets[pos_mask]  # [num_pos]

            pos_target_logits = pos_logits.gather(1, pos_targets.unsqueeze(1)).squeeze(1)
            max_logits = pos_logits.max(dim=-1)[0]
        ...
        if neg_mask.any():
            # Negative samples
            neg_logits = binary_logits[neg_mask]  # [num_neg, 2]
            neg_targets = targets[neg_mask]  # [num_neg]

            neg_target_logits = neg_logits.gather(1, neg_targets.unsqueeze(1)).squeeze(1)
            max_logits = neg_logits.max(dim=-1)[0]
