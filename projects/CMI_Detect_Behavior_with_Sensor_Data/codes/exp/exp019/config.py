...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_bert_integration", description="実験名")
    description: str = Field(
        default="BERT integration for enhanced temporal feature extraction",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["bert", "attention", "transformer", "squeezeformer", "pytorch_lightning", "physics_features"],
        description="実験タグ",
    )
...
class ModelConfig(BaseModel):
    ...
    input_dim: int = Field(default=16, description="入力次元数（基本IMU 7 + 物理特徴量 9）")
