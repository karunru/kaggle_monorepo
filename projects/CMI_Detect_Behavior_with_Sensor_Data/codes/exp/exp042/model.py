"""IMU-only LSTM with ResidualSE-ConvNeXt and BiGRU attention - exp041用 (ConvNeXt1D architecture with SE attention)."""
...
class IMUOnlyLSTM(nn.Module):
    """IMU-only LSTM model with ResidualSE-ConvNeXt and BiGRU attention (exp041: ConvNeXt1D architecture)."""
    ...
    def __init__(
        ...
        demographics_dim=0,
        dropout=0.1,
        # IMU Block configuration
        imu_block1_out_channels=64,
        imu_block1_kernel_size=3,
        imu_block1_num_blocks=2,
        imu_block1_drop_path=0.1,
        imu_block2_out_channels=128,
        imu_block2_kernel_size=5,
        imu_block2_num_blocks=2,
        imu_block2_drop_path=0.15,
        ...
        self.imu_block1 = ResidualSEConvNeXtBlock(
            in_channels=imu_dim,
            out_channels=imu_block1_out_channels,
            kernel_size=imu_block1_kernel_size,
            num_blocks=imu_block1_num_blocks,
            drop_path=imu_block1_drop_path,
            dropout=0.3,
            weight_decay=weight_decay,
        )
        self.imu_block2 = ResidualSEConvNeXtBlock(
            in_channels=imu_block1_out_channels,
            out_channels=imu_block2_out_channels,
            kernel_size=imu_block2_kernel_size,
            num_blocks=imu_block2_num_blocks,
            drop_path=imu_block2_drop_path,
            dropout=0.3,
            weight_decay=weight_decay,
        )

        # BiGRU
        self.bigru = nn.GRU(imu_block2_out_channels, bigru_hidden_size, bidirectional=True, batch_first=True)
        self.gru_dropout = nn.Dropout(gru_dropout)

        # Attention
        self.attention = AttentionLayer(bigru_hidden_size * 2)  # bidirectional

        # Dense layers (基本特徴量抽出)
        self.dense1 = nn.Linear(bigru_hidden_size * 2, 256, bias=False)
        self.bn_dense1 = nn.BatchNorm1d(256)
        self.drop1 = nn.Dropout(dense1_dropout)

        self.dense2 = nn.Linear(256, 128, bias=False)
        self.bn_dense2 = nn.BatchNorm1d(128)
        self.drop2 = nn.Dropout(dense2_dropout)
    ...
    def forward(self, x, demographics_embedding=None):
        ...
        Returns:
            tuple: (multiclass_logits, binary_logits, nine_class_logits, orientation_logits)
        """
...
class CMISqueezeformer(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,  # 他の引数は無視（後方互換性のため）
        ...
        Args:
            ...
            model_config: モデル設定（IMUOnlyLSTMのパラメータ）
            target_gestures: ターゲットジェスチャーのリスト
            non_target_gestures: ノンターゲットジェスチャーのリスト
        ...
        if model_config is not None:
            self.model = IMUOnlyLSTM(
                ...
                imu_block1_out_channels=model_config.imu_block1_out_channels,
                imu_block1_kernel_size=model_config.imu_block1_kernel_size,
                imu_block1_num_blocks=model_config.imu_block1_num_blocks,
                imu_block1_drop_path=model_config.imu_block1_drop_path,
                imu_block2_out_channels=model_config.imu_block2_out_channels,
                imu_block2_kernel_size=model_config.imu_block2_kernel_size,
                imu_block2_num_blocks=model_config.imu_block2_num_blocks,
                imu_block2_drop_path=model_config.imu_block2_drop_path,
                bigru_hidden_size=model_config.bigru_hidden_size,
    ...
    def set_orientation_prior_from_counts(self, counts_c_given_o: torch.Tensor, smoothing: float = 1.0):
        ...
        self.register_buffer("orientation_prior", prior, persistent=False)  # [O,9]
    ...
    def fuse_to_9class(
        ...
        if not hasattr(self, "orientation_prior"):
            ...
            self.register_buffer("orientation_prior", prior, persistent=False)
    ...
    def _setup_loss_functions(self):
        ...
        if loss_type == "cmi":
            ...
            self.orientation_criterion = nn.CrossEntropyLoss(label_smoothing=label_smoothing)
            ...
            # Orientation用もFocalLoss（4クラス）
            self.orientation_criterion = FocalLoss(
                gamma=self.loss_config.focal_gamma,
                alpha=self.loss_config.focal_alpha,
                label_smoothing=self.loss_config.label_smoothing,
            )
            ...
            # Orientation用もSoftF1Loss（4クラス）
            self.orientation_criterion = self._create_soft_f1_criterion(4)
            ...
            # Orientation用もSoftF1Loss with ACLS（4クラス）
            self.orientation_criterion = self._create_soft_f1_acls_criterion(4)
            ...
            # Orientation用もACLS（4クラス）
            self.orientation_criterion = self._create_acls_criterion(4)
            ...
            # Orientation用もLabelSmoothingCrossEntropy
            self.orientation_criterion = LabelSmoothingCrossEntropy(alpha=alpha)
            ...
            # Orientation用もMbLS
            self.orientation_criterion = MbLS(margin=margin, alpha=alpha, alpha_schedule=alpha_schedule)
            ...
            self.binary_criterion = nn.BCEWithLogitsLoss()
            self.nine_class_criterion = nn.CrossEntropyLoss(label_smoothing=label_smoothing)
            # Orientation用もlabel_smoothing適用
            self.orientation_criterion = nn.CrossEntropyLoss(label_smoothing=label_smoothing)
    ...
    def training_step(self, batch: dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        orientation_labels = batch["orientation_label"]
        ...
        if aw_type == "direct":
            ...
            total_loss = (
                ...
                + kl_weight * kl_loss
                + self.orient_loss_weight * orientation_loss
            )
        else:
            # 従来の固定重み損失
            total_loss = (
        ...
        self.log("train_orientation_loss", orientation_loss)
    ...
    def validation_step(self, batch: dict[str, torch.Tensor], batch_idx: int):
        ...
        self.log("val_orientation_loss", orientation_loss)

        return output

    def on_validation_epoch_start(self):
        """検証エポック開始時の処理（Schedule-Free Optimizer対応）."""
    ...
    def on_validation_epoch_end(self):
        ...
        all_orientation_probs = torch.cat([x["orientation_probs"] for x in self.validation_outputs])
        ...
        all_orientation_labels = torch.cat([x["orientation_labels"] for x in self.validation_outputs])
        ...
        try:
            # 融合による9クラス予測
            if hasattr(self, "target_idx18") and hasattr(self, "orientation_prior"):
                # 融合処理（logit和 + orientation事前）
                ...
                # 9クラス→18クラスのマッピング（0-7はそのまま、8は適当なnon-targetに）
                # ここでは最初のnon-targetジェスチャー（8）にマッピング
                multiclass_preds = torch.where(fused_preds_9 == 8, torch.tensor(8), fused_preds_9)

                print(f"Used score fusion for prediction. Fusion shape: {fused_p9.shape}")
            else:
                # フォールバック: 従来のargmax予測
                multiclass_preds = torch.argmax(all_multiclass_probs, dim=-1)
                print("Warning: Fusion indices not set, using traditional argmax prediction")
        ...
        # Orientation精度の計算
        try:
            orientation_preds = torch.argmax(all_orientation_probs, dim=-1)
            orientation_acc = (orientation_preds == all_orientation_labels).float().mean()
            self.log("val_orient_acc", orientation_acc, prog_bar=True)
        except Exception as e:
            print(f"Orientation accuracy calculation failed: {e}")

        # Schedule-Free Optimizer対応：validation終了後にtrain modeに戻す
        if self.schedule_free_config.enabled:
            optimizer = self.optimizers()
    ...
    def _calculate_orientation_prior(self, train_df):
        """
        訓練データからorientation事前確率を計算する。

        Args:
            train_df: 訓練データフレーム（gesture_id_18, orientation_idが必要）
        """

        # 18クラス→9クラスマッピング（0-7はそのまま、8-17は8(non)に統合）
        def gesture18_to9(arr18):
            return np.where(arr18 < 8, arr18, 8).astype(int)

        y9 = gesture18_to9(train_df["gesture_id_18"].to_numpy())
        o = train_df["orientation_id"].to_numpy().astype(int)

        # 共起行列を作成
        counts = np.zeros((4, 9), dtype=np.int64)  # 4姿勢 × 9クラス
        for oo, cc in zip(o, y9):
            counts[oo, cc] += 1

        counts_tensor = torch.tensor(counts, dtype=torch.float32)

        # 融合インデックスとorientation事前確率を設定
        target_idx18 = list(range(8))  # 0-7がターゲット
        self.set_fusion_indices(target_idx18, non_idx9=8)
        self.set_orientation_prior_from_counts(counts_tensor, smoothing=1.0)

        print(f"Orientation prior calculated with shape: {counts_tensor.shape}")
        print(f"Target indices set: {target_idx18}")

    def on_fit_start(self):
        """
        訓練開始時の処理。
        orientation事前確率を計算・設定する。
        """
        # DataModuleから訓練データを取得してorientation事前確率を計算
        try:
            datamodule = self.trainer.datamodule
            if hasattr(datamodule, "train_df"):
                train_df = datamodule.train_df
                self._calculate_orientation_prior(train_df)
            elif hasattr(datamodule, "train_dataloader"):
                # Datasetから直接取得する場合の処理
                train_dataset = datamodule.train_dataloader().dataset
                if hasattr(train_dataset, "df"):
                    train_df = train_dataset.df
                    self._calculate_orientation_prior(train_df)
                else:
                    print("Warning: Cannot access training data for orientation prior calculation")
                    # フォールバック: デフォルトのインデックス設定
                    self.set_fusion_indices(list(range(8)), non_idx9=8)
            else:
                print("Warning: DataModule does not have train_df attribute")
                # フォールバック: デフォルトのインデックス設定
                self.set_fusion_indices(list(range(8)), non_idx9=8)
        except Exception as e:
            print(f"Warning: Failed to calculate orientation prior: {e}")
            # フォールバック: デフォルトのインデックス設定
            self.set_fusion_indices(list(range(8)), non_idx9=8)
