...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_residual_fno_parallel", description="実験名")
    description: str = Field(
        default="Parallel ResidualSECNN and FNO-1D blocks with concatenation for multi-scale temporal feature extraction",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
