...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_physics_features", description="実験名")
    description: str = Field(
        default="Physics-based features without wavelet transforms for IMU data analysis",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["physics_features", "no_wavelets", "squeezeformer", "pytorch_lightning", "imu_analysis"],
        description="実験タグ",
    )
...
class ModelConfig(BaseModel):
    ...
    input_dim: int = Field(default=61, description="入力次元数（基本IMU 7 + 基本物理特徴量 9 + 高度な特徴量 45）")
...
class AugmentationConfig(BaseModel):
    ...
    # 利き手反転オーグメンテーション
    enable_handedness_flip: bool = Field(default=True, description="利き手反転オーグメンテーションを有効にする")
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(
        default=[EXP_NUM, "physics_features", "no_wavelets", "squeezeformer"], description="WandBタグ"
    )
...
class Config(BaseSettings):
    ...
    # bert: BertConfig = Field(default_factory=BertConfig, description="BERT設定")  # exp020では使用しない
