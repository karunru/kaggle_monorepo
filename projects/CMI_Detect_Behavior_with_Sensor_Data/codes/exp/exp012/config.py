"""pydantic-settings based configuration for exp012（ACLS統合版）."""
...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_acls_integration", description="実験名")
    description: str = Field(
        default="Integrated ACLS (Adaptive and Conditional Label Smoothing) for improved model calibration",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["acls", "calibration", "label_smoothing", "squeezeformer", "pytorch_lightning", "physics_features"],
        description="実験タグ",
    )
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(default=[EXP_NUM, "acls", "calibration", "squeezeformer"], description="WandBタグ")
