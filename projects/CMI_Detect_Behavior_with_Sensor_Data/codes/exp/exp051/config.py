...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_convnext_se_integration", description="実験名")
    description: str = Field(
        default="4-layer ConvNeXt1D blocks with SE attention",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
...
class ModelConfig(BaseModel):
    ...
    # Additional CNN block parameters for 4-layer architecture
    cnn_block3_out_channels: int = Field(default=256, description="第3CNNブロックの出力チャンネル数")
    cnn_block3_kernel_size: int = Field(default=7, description="第3CNNブロックのカーネルサイズ")
    cnn_block4_out_channels: int = Field(default=512, description="第4CNNブロックの出力チャンネル数")
    cnn_block4_kernel_size: int = Field(default=9, description="第4CNNブロックのカーネルサイズ")
...
class EMAConfig(BaseModel):
    ...
    enabled: bool = Field(default=False, description="EMA使用の有効/無効")
    beta: float = Field(default=0.9999, description="指数移動平均の減衰率")
    update_every: int = Field(default=1, description="更新頻度")
    update_after_step: int = Field(default=100, description="更新開始ステップ数")
...
class TrainingConfig(BaseModel):
    ...
    batch_size: int = Field(default=128 * 2, description="バッチサイズ")
