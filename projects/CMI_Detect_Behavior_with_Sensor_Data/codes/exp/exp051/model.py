"""IMU-only LSTM with ResidualSE-ConvNeXt blocks and BiGRU attention."""
...
class IMUOnlyLSTM(nn.Module):
    ...
    def __init__(
        ...
        if model_config is None:
            ...
            cnn_block3_out = model_config.cnn_block3_out_channels
            cnn_block4_out = model_config.cnn_block4_out_channels
            cnn_block1_kernel = model_config.cnn_block1_kernel_size
            cnn_block2_kernel = model_config.cnn_block2_kernel_size
            cnn_block3_kernel = model_config.cnn_block3_kernel_size
            cnn_block4_kernel = model_config.cnn_block4_kernel_size
        ...
        # IMU deep branch with ResidualSE-ConvNeXt blocks (4-layer architecture)
        ...
        self.imu_block3 = ResidualSEConvNeXtBlock(
            cnn_block2_out,
            cnn_block3_out,
            cnn_block3_kernel,
            dropout=cnn_dropout,
        ...
        self.imu_block4 = ResidualSEConvNeXtBlock(
            cnn_block3_out,
            cnn_block4_out,
            cnn_block4_kernel,
            dropout=cnn_dropout,
        ...
        # BiGRU
        self.bigru = nn.GRU(cnn_block4_out, lstm_hidden_dim, bidirectional=True, batch_first=True)
    ...
    def forward(self, x, demographics_embedding=None):
        ...
        # IMU branch with ResidualSE-CNN (4-layer architecture)
        x1 = self.imu_block1(imu)
        x1 = self.imu_block2(x1)
        x1 = self.imu_block3(x1)
        x1 = self.imu_block4(x1)
...
class CMISqueezeformer(pl.LightningModule):
    ...
    def training_step(self, batch: dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # EMAの更新
        if self.ema_config.enabled and self.ema is not None:
            self.ema.update()
    ...
    def on_train_epoch_end(self):
        """訓練エポック終了時にEMAモデルで更新."""
        if self.ema_config.enabled and self.ema is not None:
            self.ema.update_model_with_ema()
