"""pydantic-settings based configuration for exp010（物理ベースIMU特徴量追加版）."""
...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_physics_based_imu_features", description="実験名")
    description: str = Field(
        default="Added physics-based IMU features (linear acceleration, angular velocity, angular distance)",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["imu_physics", "squeezeformer", "pytorch_lightning", "physics_features"],
        description="実験タグ",
    )
...
class LossConfig(BaseModel):
    ...
    type: Literal["cmi", "cmi_focal", "soft_f1"] = Field(default="cmi", description="損失関数タイプ")
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(default=[EXP_NUM, "switch_ema", "squeezeformer"], description="WandBタグ")
