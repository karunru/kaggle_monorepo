"""IMU特化Squeezeformerモデル - exp014用（MiniRocket統合版）."""
...
class CMISqueezeformer(pl.LightningModule):
    ...
    def __init__(
        self,
        input_dim: int = 352,  # exp014: 基本IMU(16) + MiniRocket(336) = 352次元
        d_model: int = 256,
        n_layers: int = 8,
        n_heads: int = 8,
        d_ff: int = 1024,
        ...
        Args:
            input_dim: 入力次元数（exp014: 基本IMU 16次元 + MiniRocket 336次元 = 352次元）
            d_model: モデル次元
            n_layers: レイヤー数
            n_heads: アテンションヘッド数
            d_ff: フィードフォワード次元
...
if __name__ == "__main__":
    """テスト実行."""
    # モデルのテスト（exp014: 基本IMU 16次元 + MiniRocket 336次元 = 352次元）
    model = CMISqueezeformer(input_dim=352, d_model=256, n_layers=4, n_heads=8, d_ff=1024, num_classes=18)

    # テストデータ（exp014拡張特徴量）
    batch_size = 2
    seq_len = 200
    imu_data = torch.randn(batch_size, 352, seq_len)  # 拡張された特徴量次元
