...
# MiniRocket imports for time series transformation
import pandas as pd
import polars as pl
...
class IMUDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            rocket_config: MiniRocketMultivariate設定（オプショナル）
        """
        self.df = df
        ...
        self.rocket_config = rocket_config or {}
        self.use_rocket = self.rocket_config.get("enabled", True)
        self.rocket_transformer = None
        self.rocket_fitted = False

        if self.use_demographics:
            print(f"Loading demographics data with {len(demographics_data)} subjects...")
            # subjectをキーとしたdictsに変換（高速アクセス用）
        ...
        # MiniRocket変換対象の9つの時系列（exp014で指定）
        self.rocket_target_cols = [
            "linear_acc_x",
            "linear_acc_y",
            "linear_acc_z",
            "linear_acc_mag",
    ...
    def _setup_minirocket(self):
        """MiniRocketMultivariate変換器の初期化."""
        if not self.use_rocket:
            return

        print("Setting up MiniRocketMultivariate transformer...")

        # 設定パラメータ
        num_kernels = self.rocket_config.get("num_kernels", 336)  # 調査結果に基づく推奨値
        n_jobs = self.rocket_config.get("n_jobs", -1)  # 全CPUコア使用
        random_state = self.rocket_config.get("random_state", 42)  # 再現性確保

        # MiniRocketMultivariate初期化
        self.rocket_transformer = MiniRocketMultivariate(
            num_kernels=num_kernels, n_jobs=n_jobs, random_state=random_state
        )

        print(f"MiniRocketMultivariate initialized with num_kernels={num_kernels}")

    def _fit_minirocket(self, rocket_data_df: pd.DataFrame):
        """MiniRocket変換器を学習データでfit."""
        if not self.use_rocket or self.rocket_fitted:
            return

        print("Fitting MiniRocketMultivariate transformer...")
        start_time = time.time()

        try:
            self.rocket_transformer.fit(rocket_data_df)
            self.rocket_fitted = True
            fit_time = time.time() - start_time
            print(f"MiniRocket fit completed in {fit_time:.2f} seconds")
        except Exception as e:
            print(f"Warning: MiniRocket fit failed: {e}")
            self.use_rocket = False

    def _transform_minirocket(self, rocket_data_df: pd.DataFrame) -> np.ndarray | None:
        """MiniRocket変換を実行."""
        if not self.use_rocket or not self.rocket_fitted:
            return None

        try:
            transformed = self.rocket_transformer.transform(rocket_data_df)
            # pandasの場合、numpy配列に変換
            if hasattr(transformed, "values"):
                return transformed.values
            return transformed
        except Exception as e:
            print(f"Warning: MiniRocket transform failed: {e}")
            return None

    def _prepare_rocket_data_for_sequence(self, sequence_data: dict) -> pd.DataFrame | None:
        """特定のシーケンスのMiniRocket用データを準備."""
        if not self.use_rocket:
            return None

        try:
            # IMUデータから対象の9つの時系列を取得
            imu_data = sequence_data["imu"]  # [features, seq_len]

            # 対象列のインデックスを取得
            rocket_indices = []
            for col in self.rocket_target_cols:
                if col in self.imu_cols:
                    rocket_indices.append(self.imu_cols.index(col))

            if not rocket_indices:
                return None

            # 対象データを抽出 [target_features, seq_len]
            rocket_data = imu_data[rocket_indices]

            # sktime形式のDataFrame作成（MultiIndex）
            data_rows = []
            for feature_idx, feature_name in enumerate(self.rocket_target_cols):
                if feature_idx < len(rocket_indices):
                    time_series = rocket_data[feature_idx]
                    for time_idx, value in enumerate(time_series):
                        data_rows.append(
                            {
                                "sample": 0,  # 単一シーケンス
            ...
            print(f"Warning: Failed to prepare rocket data for sequence: {e}")
            return None

    def _fit_minirocket_on_all_data(self):
        """全シーケンスデータを使ってMiniRocket変換器をfit."""
        if not self.use_rocket:
            return

        print("Preparing all sequences data for MiniRocket fit...")

        # 全シーケンスのrocketデータを収集
        all_rocket_data = []

        for seq_idx, seq_id in enumerate(self.sequence_ids):
            if seq_id in self.sequence_data:
                sequence_data = self.sequence_data[seq_id]

                # 単一シーケンス用のrocketデータを準備
                rocket_df = self._prepare_rocket_data_for_sequence(sequence_data)
                if rocket_df is not None:
                    # サンプルインデックスを更新（全データで一意になるように）
                    rocket_df = rocket_df.reset_index()
                    rocket_df["sample"] = seq_idx
                    rocket_df = rocket_df.set_index(["sample", "time"])
                    all_rocket_data.append(rocket_df)

            if (seq_idx + 1) % 100 == 0:
                print(f"Processed {seq_idx + 1}/{len(self.sequence_ids)} sequences for MiniRocket")

        if not all_rocket_data:
            print("Warning: No valid rocket data found, disabling MiniRocket")
            self.use_rocket = False
            return

        # 全データを結合
        print(f"Combining {len(all_rocket_data)} rocket datasets...")
        combined_rocket_df = pd.concat(all_rocket_data, axis=0)

        print(f"Combined rocket data shape: {combined_rocket_df.shape}")
        print(f"Features: {list(combined_rocket_df.columns)}")

        # MiniRocket変換器をfit
        self._fit_minirocket(combined_rocket_df)

    def _get_rocket_features_for_sequence(self, sequence_data: dict) -> np.ndarray | None:
        """特定のシーケンスに対するMiniRocket特徴量を取得."""
        if not self.use_rocket or not self.rocket_fitted:
            return None
        ...
        try:
            # シーケンス用のrocketデータを準備
            rocket_df = self._prepare_rocket_data_for_sequence(sequence_data)
            if rocket_df is None:
                return None

            # MiniRocket変換を実行
            rocket_features = self._transform_minirocket(rocket_df)
            if rocket_features is not None and len(rocket_features) > 0:
                return rocket_features[0]  # 単一シーケンスの特徴量を返す
            return None

        except Exception as e:
            # デバッグ用の詳細ログ（最初の数回のみ）
            if not hasattr(self, "_rocket_error_count"):
                self._rocket_error_count = 0
            if self._rocket_error_count < 3:
                print(f"Warning: Failed to get rocket features for sequence: {e}")
                self._rocket_error_count += 1
            return None
    ...
    def _preprocess_data(self):
        ...
        # MiniRocket変換器のfit処理
        if self.use_rocket:
            self._fit_minirocket_on_all_data()
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        # MiniRocket特徴量の生成と統合
        if self.use_rocket and self.rocket_fitted:
            rocket_features = self._get_rocket_features_for_sequence(data)
            if rocket_features is not None:
                # Rocket特徴量を時系列次元に拡張: [num_kernels] -> [num_kernels, seq_len]
                seq_len = imu_tensor.shape[1]
                rocket_tensor = torch.tensor(rocket_features, dtype=torch.float32)
                rocket_expanded = rocket_tensor.unsqueeze(-1).expand(-1, seq_len)  # [num_kernels, seq_len]

                # 元の時系列とRocket特徴量を結合
                imu_tensor = torch.cat(
                    [imu_tensor, rocket_expanded], dim=0
                )  # [orig_features + rocket_features, seq_len]
        ...
        if self.use_demographics:
            ...
            demographics = self._get_demographics_for_subject(subject)
