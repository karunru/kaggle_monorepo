"""IMU-only LSTM訓練スクリプト - exp029用(Human Normalization + Demographics + multi-head KL divergence)."""
...
def train_single_fold(config: Config, fold: int, logger, wandb_logger) -> dict:
    ...
    # Setup data module to create datasets
    data_module.setup("fit")

    # Get actual input dimension from dataset (includes HN features if enabled)
    actual_input_dim = len(data_module.train_dataset.imu_cols)
    logger.info(f"Using input_dim={actual_input_dim} (HN features: {config.demographics.hn_enabled})")

    # IMU-only LSTM Model (exp028 compatible)
    ...
    # Clear previous fold validation results
    model.clear_fold_results()
    ...
    # Save fold validation results to CSV
    try:
        model.save_fold_validation_results_to_csv(fold)
    except Exception as e:
        logger.warning(f"Failed to save fold {fold} validation results to CSV: {e}")
...
def train_cross_validation(config: Config) -> dict:
    """クロスバリデーション訓練."""
    # ロガーの設定
    log_dir = config.paths.output_dir / "logs"
    ...
    results_path = config.paths.output_dir / "cv_results.csv"
