...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_mish_bert_integration", description="実験名")
    description: str = Field(
        default="Hierarchical Bayesian fusion of three classification heads with configurable IMU parameters",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
...
class PathsConfig(BaseModel):
    ...
    output_dir: Path = Field(default=Path(f"../../../outputs/{EXP_NUM}"), description="出力ディレクトリ")
...
class ModelConfig(BaseModel):
    ...
    imu_block1_out_channels: int = Field(default=64, description="IMUブロック1出力チャンネル数")
    imu_block2_out_channels: int = Field(default=128, description="IMUブロック2出力チャンネル数")
    bigru_hidden_size: int = Field(default=128, description="BiGRU隠れ層サイズ")
    dense1_out_features: int = Field(default=256, description="Dense1層出力特徴量数")
    dense2_out_features: int = Field(default=128, description="Dense2層出力特徴量数")
    dense1_dropout: float = Field(default=0.5, description="Dense1層ドロップアウト率")
    dense2_dropout: float = Field(default=0.3, description="Dense2層ドロップアウト率")
    gru_dropout: float = Field(default=0.4, description="GRUドロップアウト率")
    imu_block_dropout: float = Field(default=0.3, description="IMUブロックドロップアウト率")

    # Probability fusion configurations
    fusion_temperature_18: float = Field(default=1.0, description="18クラス分布の温度パラメータ")
    fusion_temperature_binary: float = Field(default=1.0, description="バイナリ分布の温度パラメータ")
    fusion_temperature_9: float = Field(default=1.0, description="9クラス分布の温度パラメータ")
    fusion_weight_hierarchical: float = Field(default=0.6, description="階層分布の重み")
    fusion_tau_threshold: float = Field(default=0.5, description="バイナリしきい値")
...
class AugmentationConfig(BaseModel):
    ...
    randaugment_n: int = Field(default=2, description="RandAugment: 適用する変換数")
    randaugment_m: float = Field(default=6.0, description="RandAugment: 変換強度")
    randaugment_prob: float = Field(default=0.8, description="RandAugment適用確率")
...
class Config(BaseSettings):
    ...
    def create_output_dirs(self) -> None:
        ...
        output_dirs = [
            ...
            self.paths.output_dir / "logs",
            self.paths.output_dir / "validation_results",
        ]
