...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_fno_implementation", description="実験名")
    description: str = Field(
        default="FNO-1D blocks for long-range temporal dependencies with IMU-only LSTM, human normalization, and demographics",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
