...
class ExperimentConfig(BaseModel):
    ...
    description: str = Field(
        default="tsai transforms with RandAugment - IMU rotation and handedness flip",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
...
class TrainingConfig(BaseModel):
    ...
    # 基本設定
    seed: int = Field(default=505, description="ランダムシード")
...
class ValidationConfigParams(BaseModel):
    ...
    random_state: int = Field(default=505, description="ランダムステート")
