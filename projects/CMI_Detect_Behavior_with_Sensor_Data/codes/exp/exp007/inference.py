...
print(f"{str(Path(__file__).resolve().parents[2])=}")
...
def get_best_checkpoint(checkpoint_dir: Path) -> Path | None:
    """val_cmi_scoreが最高のcheckpointを取得."""
    ckpt_files = list(checkpoint_dir.glob("epoch-*-val_cmi_score-*.ckpt"))
    if not ckpt_files:
        return None

    # ファイル名から cmi_score を抽出して最大値を選択
    def extract_cmi_score(ckpt_path):
        filename = ckpt_path.name
        # epoch-XX-val_cmi_score-Y.YYYY.ckpt から Y.YYYY を抽出
        parts = filename.split("-")
        for i, part in enumerate(parts):
            if part == "val_cmi_score" and i + 1 < len(parts):
                score_part = parts[i + 1].replace(".ckpt", "")
                return float(score_part)
        return 0.0

    best_ckpt = max(ckpt_files, key=extract_cmi_score)
    return best_ckpt
