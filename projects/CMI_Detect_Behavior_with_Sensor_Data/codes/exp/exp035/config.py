...
class ExperimentConfig(BaseModel):
    ...
    description: str = Field(
        default="BERT integration with Mish activation - head前にBERT層を追加",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
...
class BertConfig(BaseModel):
    ...
    num_layers: int = Field(default=2, description="BERTレイヤー数")
    num_heads: int = Field(default=4, description="BERTアテンションヘッド数")
