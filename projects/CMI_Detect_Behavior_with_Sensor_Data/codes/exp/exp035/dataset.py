"""IMU-only dataset module with jiazhuang notebook features - exp029用（Human Normalization + Demographics + Nine-class labels）."""
...
class IMUDataset(Dataset):
    ...
    def _apply_augmentation(self, imu_data: np.ndarray, subject_id: str = None) -> tuple[np.ndarray, bool]:
        ...
        # ガウシアンノイズ
        if self.augmentation_config and np.random.random() < self.augmentation_config.gaussian_noise_prob:
        ...
        # 時間スケーリング
        if self.augmentation_config and np.random.random() < self.augmentation_config.time_scaling_prob:
        ...
        # 部分マスキング
        if self.augmentation_config and np.random.random() < self.augmentation_config.partial_masking_prob:
...
class TimeSeriesAugmentation:
    """Time series data augmentation for IMU sensor data."""
    ...
    def __init__(
        ...
        """
        Initialize time series augmentation parameters.

        Args:
            time_stretch_range: Time stretching range (min_ratio, max_ratio)
            time_shift_range: Time shifting range (ratio of sequence length)
            noise_std: Gaussian noise standard deviation
            magnitude_scale_range: Magnitude scaling range
            rotation_angle_range: Rotation angle range (radians)
            mask_ratio: Time masking ratio
            freq_filter_range: Frequency filtering range
            aug_prob: Application probability for each augmentation method
        """
        ...
        # IMU feature indices (first 7 features: acc_x, acc_y, acc_z, rot_w, rot_x, rot_y, rot_z)
        self.imu_indices = list(range(7))
        self.acc_indices = [0, 1, 2]  # acceleration
        self.rot_indices = [3, 4, 5, 6]  # quaternion rotation

    def __call__(self, sequence):
        """Apply random augmentation to sequence."""
        augmented = sequence.copy()

        # Randomly apply various augmentation methods
    ...
    def time_stretch(self, sequence):
        ...
        for i in range(num_features):
            interpolator = interpolate.interp1d(
                original_time, sequence[:, i], kind="linear", bounds_error=False, fill_value="extrapolate"
            )
    ...
    def magnitude_scale(self, sequence):
        """Magnitude scaling."""
        scale_factor = random.uniform(*self.magnitude_scale_range)
        scaled = sequence.copy()

        # Scale only IMU data, keep other features unchanged
        scaled[:, self.imu_indices] *= scale_factor

        return scaled

    def rotate_imu(self, sequence):
        """Apply small angle rotation to IMU data."""
        rotated = sequence.copy()

        # Generate small angle rotation matrix
        angle = random.uniform(-self.rotation_angle_range, self.rotation_angle_range)
        cos_a, sin_a = np.cos(angle), np.sin(angle)

        # Rotate only acceleration data (simplified version, only xy plane rotation)
    ...
    def time_mask(self, sequence):
        """Time masking - randomly mask a time window."""
        seq_len, num_features = sequence.shape
        ...
        # Fill masked region with average of before/after values
