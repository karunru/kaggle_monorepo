"""pydantic-settings based configuration for exp006（Switch EMA統合）."""
...
class ExperimentConfig(BaseModel):
    ...
    name: str = Field(default=f"{EXP_NUM}_switch_ema_squeezeformer", description="実験名")
    description: str = Field(default="Switch EMA integrated Squeezeformer", description="実験説明")
    tags: list[str] = Field(
        default=["imu_only", "squeezeformer", "pytorch_lightning", "switch_ema"],
        description="実験タグ",
    )
...
class LengthGroupingConfig(BaseModel):
    ...
    @validator("percentile_max_length")
    def validate_percentile(cls, v):
        if not 0 < v <= 1:
            raise ValueError("percentile_max_length must be between 0 and 1")
        return v

    @validator("mega_batch_multiplier")
    def validate_mega_batch_mult(cls, v):
        if v < 1:
            raise ValueError("mega_batch_multiplier must be at least 1")
        return v
...
class EMAConfig(BaseModel):
    ...
    @validator("update_after_step")
    def validate_update_after_step(cls, v):
        if v < 0:
            raise ValueError("update_after_step must be non-negative")
        return v

    @validator("update_every")
    def validate_update_every(cls, v):
        if v < 1:
            raise ValueError("update_every must be at least 1")
        return v

    @validator("update_model_with_ema_every")
    def validate_switch_every(cls, v):
        if v < 1:
            raise ValueError("update_model_with_ema_every must be at least 1")
        return v
...
class TrainerConfig(BaseModel):
    ...
    default_root_dir: str = Field(default=f"../../outputs/{EXP_NUM}", description="デフォルトルートディレクトリ")
...
class WandbConfig(BaseModel):
    ...
    name: str = Field(default=f"{EXP_NUM}", description="実行名")
    tags: list[str] = Field(default=[EXP_NUM, "switch_ema", "squeezeformer"], description="タグ")
...
class LoggingConfig(BaseModel):
    ...
    log_file: str = Field(default=f"{EXP_NUM}.log", description="ログファイル名")
...
class Config(BaseSettings):
    ...
    # Root directory
    root: str = Field(default=f"../../outputs/{EXP_NUM}", description="ルートディレクトリ")
    ...
    class Config:
        ...
        env_prefix = "EXP005_"  # 環境変数プレフィックス
