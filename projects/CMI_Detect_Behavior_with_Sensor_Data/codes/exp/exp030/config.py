...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_softf1_acls", description="実験名")
    description: str = Field(
        default="SoftF1ACLS implementation with IMU-only LSTM, human normalization, and demographics",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=[
...
class LossConfig(BaseModel):
    ...
    type: Literal["cmi", "cmi_focal", "soft_f1", "soft_f1_acls", "acls", "label_smoothing", "mbls"] = Field(
        default="soft_f1_acls", description="損失関数タイプ"
    )
