...
class IMUDataset(Dataset):
    ...
    def _preprocess_data(self):
        ...
        # Update sequence_ids to match the actual sequences in sequence_data
        if self.filter_gesture_only:
            original_count = len(self.sequence_ids)
            self.sequence_ids = list(self.sequence_data.keys())
            filtered_count = original_count - len(self.sequence_ids)
            if filtered_count > 0:
                print(f"Filtered out {filtered_count} sequences that have no Gesture phase data")
                print(f"Remaining sequences: {len(self.sequence_ids)}")
    ...
    def _preprocess_data_vectorized_with_mask(self, df: pl.DataFrame | None = None) -> dict:
        ...
        df_with_physics = self._add_physics_features(df)
