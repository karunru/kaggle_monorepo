...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_focal_loss_nine_class", description="実験名")
    description: str = Field(
        default="FocalLoss for both multiclass and nine-class heads",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["focal_loss", "nine_class_focal", "multiclass_consistency", "squeezeformer", "pytorch_lightning"],
        description="実験タグ",
    )
...
class BertConfig(BaseModel):
    ...
    hidden_size: int | None = Field(default=None, description="BERT隠れ層サイズ (Noneでd_modelを使用)")
    num_layers: int = Field(default=4, description="BERTレイヤー数")
    num_heads: int = Field(default=8, description="BERTアテンションヘッド数")
    intermediate_size: int | None = Field(default=None, description="BERT中間層サイズ (Noneでhidden_size*4を使用)")
...
class TrainingConfig(BaseModel):
    ...
    learning_rate: float = Field(default=1e-4, description="学習率（NaN防止のため下げた）")
...
class LossConfig(BaseModel):
    ...
    type: Literal["cmi", "cmi_focal", "soft_f1", "acls", "label_smoothing", "mbls"] = Field(
        default="soft_f1", description="損失関数タイプ"
    )
    ...
    # 学習可能重み付け設定
    auto_weighting: Literal["none", "uncertainty", "direct"] = Field(default="direct", description="自動重み付け方式")
...
class AugmentationConfig(BaseModel):
    ...
    handedness_flip_prob: float = Field(default=0.5, description="利き手反転適用確率")
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(
        default=[EXP_NUM, "focal_loss", "nine_class_focal", "multiclass_consistency", "squeezeformer"],
        description="WandBタグ",
    )
...
class Config(BaseSettings):
    ...
    bert: BertConfig = Field(default_factory=BertConfig, description="BERT設定")
