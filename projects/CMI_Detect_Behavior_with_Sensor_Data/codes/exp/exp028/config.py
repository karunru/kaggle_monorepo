...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_imu_only_lstm_jiazhuang", description="実験名")
    description: str = Field(
        default="IMU-only LSTM with ResidualSE-CNN and BiGRU attention (based on jiazhuang notebook)",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["imu_only", "lstm", "bigru", "attention", "residual_se_cnn", "jiazhuang_baseline"],
        description="実験タグ",
    )
...
class DemographicsConfig(BaseModel):
    ...
    # 基本設定
    enabled: bool = Field(default=False, description="Demographics統合を有効にするかどうか")
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(
        default=[EXP_NUM, "imu_only", "lstm", "bigru", "attention", "residual_se_cnn"],
        description="WandBタグ",
    )
