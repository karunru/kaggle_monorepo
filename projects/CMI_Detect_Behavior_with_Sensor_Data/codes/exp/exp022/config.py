...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_human_normalization", description="実験名")
    description: str = Field(
        default="Human Normalization using anthropometrics for body-size invariant IMU features",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["human_normalization", "anthropometrics", "physics_features", "squeezeformer", "pytorch_lightning"],
        description="実験タグ",
    )
...
class LossConfig(BaseModel):
    ...
    type: Literal["cmi", "cmi_focal", "soft_f1", "acls", "label_smoothing", "mbls"] = Field(
        default="cmi", description="損失関数タイプ"
    )
