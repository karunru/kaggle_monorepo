"""IMU専用データセットモジュール - exp003用（系列長グループ化対応）."""
...
class IMUDataset(Dataset):
    ...
    def _preprocess_data_vectorized(self) -> dict:
        ...
        # 並列でシーケンス長正規化
        sequence_data = self._normalize_sequences_parallel(processed_data)

        return sequence_data

    def _normalize_sequences_parallel(self, processed_data: pl.DataFrame) -> dict:
        """並列でシーケンス長正規化."""
        sequence_data = {}
        ...
        def process_sequence_batch(batch_data: list[tuple]) -> list[tuple]:
            ...
            for row_data in batch_data:
                ...
                if not self.use_dynamic_padding:
                    normalized_imu = self._normalize_sequence_length_vectorized(imu_array)
                else:
                    normalized_imu = imu_array

                # ラベル情報
                multiclass_label = self.gesture_to_id[gesture]
                binary_label = self.is_target_gesture[gesture]

                batch_results.append((seq_id, normalized_imu, original_length, multiclass_label, binary_label, gesture))
        ...
        for batch_result in batch_results:
            for seq_id, imu_data, original_length, multiclass_label, binary_label, gesture in batch_result:
