...
# Physics-based feature extraction functions removed - now implemented in model.py (IMUFeatureExtractor)
...
class IMUDataset(Dataset):
    ...
    def __init__(
        ...
        # IMU列の定義（基本IMUデータのみ、特徴量はモデル側で生成）
        self.imu_cols = [
            "acc_x",
            "acc_y",
            "acc_z",
    ...
    # Physics features are now computed in model.py (IMUFeatureExtractor)
    ...
    def _preprocess_data_vectorized_with_mask(self) -> dict:
        ...
        print("Note: Physics features are now computed in model (IMUFeatureExtractor)")

        # 基本IMU列のみを使用（特徴量抽出はモデル側で実行）
        basic_imu_cols = ["acc_x", "acc_y", "acc_z", "rot_w", "rot_x", "rot_y", "rot_z"]
...
class SingleSequenceIMUDataset(Dataset):
    ...
    # Physics features now computed in model (removed from dataset)
