...
class IMUFeatureExtractor(nn.Module):
    """
    IMU特徴量抽出モジュール（exp013とReference Notebook統合版）.

    生のIMUデータ（7次元）から39次元の拡張特徴量を生成。
    """

    def __init__(self, sampling_rate: float = 100.0, cutoff_freq: float = 10.0, filter_order: int = 15):
        """
        初期化.

        Args:
            sampling_rate: サンプリングレート [Hz]
            cutoff_freq: フィルタのカットオフ周波数 [Hz]
        ...
        # Low-pass filter for acceleration and angular velocity (using Conv1d)
        # Normalized cutoff frequency
        nyquist = sampling_rate / 2
        normalized_cutoff = cutoff_freq / nyquist

        # Create filter coefficients (simple moving average approximation)
        # For a proper implementation, we could use firwin equivalent
        self.filter_size = filter_order

        # Low-pass filter (Conv1d with groups for parallel filtering)
        self.lpf_acc = nn.Conv1d(3, 3, kernel_size=filter_order, padding=filter_order // 2, groups=3, bias=False)
        self.lpf_ang_vel = nn.Conv1d(3, 3, kernel_size=filter_order, padding=filter_order // 2, groups=3, bias=False)

        # Initialize filter weights (moving average)
        with torch.no_grad():
            weight = torch.ones(filter_order) / filter_order
            self.lpf_acc.weight.data = weight.view(1, 1, -1).repeat(3, 1, 1)
            self.lpf_ang_vel.weight.data = weight.view(1, 1, -1).repeat(3, 1, 1)

        # Freeze filter weights
        self.lpf_acc.weight.requires_grad = False
        self.lpf_ang_vel.weight.requires_grad = False
    ...
    def remove_gravity(self, acc: torch.Tensor, quat: torch.Tensor) -> torch.Tensor:
        ...
        # 四元数を正規化
        quat_norm = torch.norm(quat, dim=1, keepdim=True) + eps
        ...
        # 重力ベクトル（センサー座標系での重力）を計算
        # 重力ベクトル（ワールド座標系）を四元数で回転
    ...
    def quaternion_to_angular_velocity(self, quat: torch.Tensor) -> torch.Tensor:
        ...
        # 四元数を正規化（より厳格な正規化）
        quat_norm = torch.norm(quat, dim=1, keepdim=True).clamp(min=eps)
        quat_normalized = quat / quat_norm

        # 時間微分を計算（forward差分でより安定）
        quat_dot = F.pad(quat_normalized[:, :, 1:] - quat_normalized[:, :, :-1], (1, 0)) / dt

        # 角速度の計算: ω = 2 * q_dot * q_conjugate
        qw, qx, qy, qz = (
            quat_normalized[:, 0:1],
            quat_normalized[:, 1:2],
            quat_normalized[:, 2:3],
        ...
        qw_dot, qx_dot, qy_dot, qz_dot = quat_dot[:, 0:1], quat_dot[:, 1:2], quat_dot[:, 2:3], quat_dot[:, 3:4]

        # 四元数の共役との積（数値的安定性を向上）
        omega_x = 2 * (-qx * qw_dot + qw * qx_dot + qz * qy_dot - qy * qz_dot)
        omega_y = 2 * (-qy * qw_dot - qz * qx_dot + qw * qy_dot + qx * qz_dot)
        omega_z = 2 * (-qz * qw_dot + qy * qx_dot - qx * qy_dot + qw * qz_dot)

        angular_vel = torch.cat([omega_x, omega_y, omega_z], dim=1)

        # NaNや無限大をクリッピング
        angular_vel = torch.clamp(angular_vel, -1000.0, 1000.0)
    ...
    def calculate_angular_distance(self, quat: torch.Tensor) -> torch.Tensor:
        ...
        # 隣接する四元数の内積を計算
        quat_prev = F.pad(quat_normalized[:, :, :-1], (1, 0), mode="replicate")
        quat_curr = quat_normalized

        # 内積計算
        dot_product = torch.sum(quat_prev * quat_curr, dim=1, keepdim=True)

        # 角距離の計算: θ = arccos(|dot_product|)
        # より厳格なクランピング（arccos定義域対応）
        dot_product_abs = torch.abs(dot_product)
        dot_product_clamped = torch.clamp(dot_product_abs, eps, 1.0 - eps)
        angular_distance = torch.acos(dot_product_clamped)

        # NaNや無限大の防止
        angular_distance = torch.nan_to_num(angular_distance, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def forward(self, imu_data: torch.Tensor) -> torch.Tensor:
        ...
        # 3. 追加特徴量 (Reference Notebookから適応) (23次元)
        # - 加速度関連特徴量: 13次元
        # - 角速度関連特徴量: 10次元
        # 加速度の大きさ
        acc_mag = torch.norm(acc, dim=1, keepdim=True)
        features_list.append(acc_mag)  # [B, 1, T]

        # 加速度のジャーク
        acc_jerk = F.pad(acc[:, :, 1:] - acc[:, :, :-1], (1, 0))
        features_list.append(acc_jerk)  # [B, 3, T]

        # 加速度のパワー（エネルギー）
        acc_power = acc**2
        features_list.append(acc_power)  # [B, 3, T]

        # 加速度のフィルタリング
        acc_lpf = self.lpf_acc(acc)
        acc_hpf = acc - acc_lpf
        features_list.append(acc_lpf)  # [B, 3, T]
        features_list.append(acc_hpf)  # [B, 3, T]

        # 角速度の大きさ
        angular_vel_mag = torch.norm(angular_vel, dim=1, keepdim=True)
        features_list.append(angular_vel_mag)  # [B, 1, T]

        # 角速度の微分
        angular_vel_delta = F.pad(angular_vel[:, :, 1:] - angular_vel[:, :, :-1], (1, 0))
        features_list.append(angular_vel_delta)  # [B, 3, T]

        # 角速度のパワー
        angular_vel_power = angular_vel**2
        features_list.append(angular_vel_power)  # [B, 3, T]

        # 角速度のフィルタリング（LPFのみ、HPFは除去してサイズを39次元に調整）
        angular_vel_lpf = self.lpf_ang_vel(angular_vel)
        features_list.append(angular_vel_lpf)  # [B, 3, T]

        # 全特徴量を結合
        features = torch.cat(features_list, dim=1)  # [B, 39, T]
...
class CMISqueezeformer(pl.LightningModule):
    ...
    def __init__(
        ...
        self.imu_feature_extractor = IMUFeatureExtractor(
            ...
            cutoff_freq=self.feature_extractor_config.get("cutoff_freq", 10.0),
            filter_order=self.feature_extractor_config.get("filter_order", 15),
        )

        # 特徴量抽出後の次元数（39次元）
        self.extracted_feature_dim = 39
        ...
        # Input projection（特徴量抽出後の39次元 → d_model）
