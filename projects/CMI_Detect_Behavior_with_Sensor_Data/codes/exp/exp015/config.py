"""pydantic-settings based configuration for exp013（Demographics統合版）."""
...
class ExperimentConfig(BaseModel):
    ...
    exp_num: str = EXP_NUM
    name: str = Field(default=f"{EXP_NUM}_pytorch_feature_extraction", description="実験名")
    description: str = Field(
        default="PyTorch-based IMU feature extraction integrated with Squeezeformer model",
        description="実験説明",
    )
    tags: list[str] = Field(
        default=["pytorch_features", "end_to_end", "acls", "squeezeformer", "pytorch_lightning", "gpu_acceleration"],
        description="実験タグ",
    )
...
class ModelConfig(BaseModel):
    ...
    input_dim: int = Field(default=7, description="入力次元数（基本IMUデータのみ、特徴量はモデル内で生成）")
...
class FeatureExtractorConfig(BaseModel):
    ...
    sampling_rate: float = Field(default=100.0, description="サンプリングレート [Hz]")
    cutoff_freq: float = Field(default=10.0, description="フィルタのカットオフ周波数 [Hz]")
    filter_order: int = Field(default=15, description="フィルタの次数")
...
class LoggingConfig(BaseModel):
    ...
    wandb_tags: list[str] = Field(
        default=[EXP_NUM, "pytorch_features", "end_to_end", "acls", "squeezeformer"], description="WandBタグ"
    )
...
class Config(BaseSettings):
    ...
    feature_extractor: FeatureExtractorConfig = Field(
        default_factory=FeatureExtractorConfig, description="特徴量抽出器設定"
    )
    ...
    imu_features: list[str] = Field(
        ...
        description="IMU特徴量列名（基本IMUデータのみ、拡張特徴量はモデル内で生成）",
    )
