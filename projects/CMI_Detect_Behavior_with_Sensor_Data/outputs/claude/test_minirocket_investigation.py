...
def test_basic_minirocket_functionality():
    ...
    # サンプルデータ生成
    print("サンプルデータ生成中...")
    df = generate_sample_multivariate_time_series(
        n_samples=50, 
        n_features=9, 
        n_timepoints=200,
        random_state=42
...
def test_different_num_kernels():
    ...
    # 結果まとめ
    print(f"\n=== パフォーマンス比較結果 ===")
    print(f"{'num_kernels':<12} {'fit_time':<10} {'trans_time':<11} {'total_time':<11} {'memory_MB':<10} {'features':<9}")
    print("-" * 70)
    
    for result in results:
        print(f"{result['num_kernels']:<12} "
...
def test_imu_realistic_data():
    ...
    transformer = MiniRocketMultivariate(
        num_kernels=recommended_num_kernels,
        n_jobs=-1,
        random_state=42
    )
...
def main():
    ...
    try:
        ...
        print(f"✅ 推奨パラメータ: num_kernels=336 (処理時間とメモリのバランス)")
        print(f"✅ PyTorchテンソルとの統合が可能")
        print(f"✅ 実装準備完了")
        
        # 推奨設定
        print(f"\nexp014実装への推奨設定:")
        print(f"  - num_kernels: 336")
        print(f"  - n_jobs: -1 (全CPUコア使用)")
        print(f"  - random_state: 42 (再現性確保)")
        print(f"  - 統合方法: Option 1 (時系列次元に追加)")
