...
class TestCMISqueezeformerBERT:
    ...
    def test_gradient_flow(self, model, sample_input):
        ...
        # 重要なパラメータに勾配が存在することを確認
        # (BERTのencoderとCLSトークン、classification headsなど)
        important_params = ["cls_token", "input_projection", "bert.encoder", "multiclass_head", "binary_head"]

        gradient_found = False
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is not None:
                # 重要なパラメータのいずれかに勾配があることを確認
                for important_param in important_params:
                    if important_param in name:
                        gradient_found = True
                        assert not torch.isnan(param.grad).any(), f"NaN gradient for {name}"
                        break

        assert gradient_found, "No gradients found in important model parameters"
