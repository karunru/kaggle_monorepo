...
# Define XDataFrame type for type hints
XDataFrame = Union[pd.DataFrame]
    ...
    def _merge(self, dataframe, merge=True):
        with timer("merge"):
            for param_dict, features in tqdm(
                zip(self.param_dict, self.features, strict=False),
            ...
            for a in agg:
                for v in var:
                    ...
                    if str(dataframe[v].dtype) == "category":
                        dataframe[new_feature] = dataframe[base_feature] - dataframe[v].astype(int)
                    else:
...
class CategoryVectorizer:
    ...
    def create_word_list(self, dataframe, col1, col2) -> list:
        ...
        for val1, val2 in zip(dataframe[col1].values, dataframe[col2].values, strict=False):
    ...
    def get_feature(self, dataframe, col1, col2, latent_vector, name="") -> XDataFrame:
        ...
        for i, val1 in enumerate(dataframe[col1].fillna(0).astype(int)):
...
class CategoryUser2Vec(BaseFeatureTransformer):
    ...
    def transform(self, dataframe: pd.DataFrame):
        ...
        # Removed xfeat dependency
        df["__user_id"] = ""
        for c in self.key:
            df["__user_id"] += c + df[c].astype(str) + " "
        gc.collect()
        df["__user_document"] = ""
        __user_document = df["__user_document"].copy()
        for c in tqdm(self.categorical_columns):
            c_series = c.replace("denoise_", "") + "_" + df[c].astype(str).replace("-1", "none") + " "
            del df[c]
            gc.collect()
    ...
    def create_documents(self, dataframe: pd.DataFrame):
        g = dataframe.groupby(["__user_id"], sort=True)
        documents = g["__user_document"].apply(lambda x: str(x).replace("[", "").replace("]", ""))
...
class CategoryUser2VecWithW2V(CategoryUser2Vec):
    ...
    def transform(self, dataframe):
        ...
        # vectorize
        documents, user_ids = self.create_documents(df)
        w2v = Word2Vec([list(val) for val in documents.to_list()], **self.w2v_params)
        ...
        features = (
            ...
            .merge(vocab_vectors.T, how="left", left_on="__user_document", right_index=True)
    ...
    def create_documents(self, dataframe: pd.DataFrame):
        g = dataframe.groupby(["__user_id"], sort=True)
        documents = g["__user_document"].apply(lambda x: x)
        user_ids = g["__user_document"].last().reset_index()["__user_id"]
        return documents, user_ids
...
class SinCos:
    ...
    def create_features(self, df):
        ...
        new_cols = [f"{self.feature_name}_{key}" for key in ["sin", "cos"]]
...
class TargetEncoder:
    ...
    def transform(self, X_: pd.DataFrame) -> np.ndarray:
        ...
        for i, (_, v_idx) in enumerate(kf.split(X)):
            converted[v_idx] = X.loc[v_idx, self.column].map(lambda x: self.class_dict[x][i])
        return converted

    def fit_transform(self, X_: pd.DataFrame, y: pd.Series | np.ndarray, column: str) -> np.ndarray:
        self.column = column
    ...
    def transform(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for c in self.text_columns:
            ...
            feature = pd.DataFrame(
                feature,
                columns=[c + "_" + self.name + f"_{i:03}" for i in range(self.n_components)],
            )
            features.append(feature)
...
class W2VFeatureTransformer:
    ...
    def __init__(
        ...
        self.text_columns = text_columns
        self.model = model if model is not None else gensim.downloader.load("word2vec-google-news-300")
        self.transformer = transformer
        self.n_components = transformer.n_components
        self.name = name
        ...
        for model_name in self.model_names:
            ...
            for c in self.text_columns:
                ...
                for i in tqdm(range(np.ceil(len(texts) / self.batch_size).astype(int))):
                    result.append(
                        np.max(
                            model(texts[i * self.batch_size : min(len(texts), (i + 1) * self.batch_size)]),
                            axis=1,
                        ),
                    )
                ...
                result = pd.DataFrame(
                    self.transformer.fit_transform(np.nan_to_num(result)),
                    columns=[f"{c}_{model_name}_{i:03}" for i in range(self.n_components)],
                )
                self.features.append(result)
        dataframe = pd.concat([dataframe] + self.features, axis=1)
        return dataframe
...
class GroupedCategoriesWord2VecFeatureTransformer:
    def __init__(
        ...
        self.transformer = TruncatedSVD(n_components=svd_size, random_state=1031) if svd_size > 1 else -1
        self.agg_dict = {
            "mean": np.mean,
            "median": np.median,
            "sum": np.sum,
    ...
    def transform(self, grouped_dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for c in self.listed_cat_columns:
            self.w2v_size = (
                int(len(set([item for sublist in grouped_dataframe[c].values.tolist() for item in sublist])) / 4)
            ...
            for func in self.agg_dict.keys():
                features[[f"{c}_{self.name}_{func}_{i}" for i in range(w2v_model.vector_size)]] = pd.DataFrame(
                    np.vstack(
                        [
                            x
                            for x in grouped_dataframe[c].progress_apply(
                                lambda x: self.agg_dict[func]([w2v_model.wv[e] for e in x], axis=0),
                            )
                        ],
                    ),
                )
            ...
            if self.transformer != -1:
                for func in self.agg_dict.keys():
                    features[[f"{c}_{self.name}_{func}_svd_{i}" for i in range(self.transformer.n_components)]] = (
                        pd.DataFrame(
                            np.vstack(
                                self.transformer.fit_transform(
                                    np.nan_to_num(
                                        features[
                                            [f"{c}_{self.name}_{func}_{i}" for i in range(w2v_model.vector_size)]
                                        ].values,
                                    ),
                                ),
                            ),
                    ...
                    features = features.drop(
                        columns=[f"{c}_{self.name}_{func}_{i}" for i in range(w2v_model.vector_size)],
                    )
    ...
    def __init__(
        ...
        *Instantiates a multilingual transformer model for translation.*
        ...
        self._transformers_model = (
            AutoModelForSeq2SeqLM.from_pretrained(self.model_or_path, **model_options).to(self.device).eval()
        )
        if model_to_half:
