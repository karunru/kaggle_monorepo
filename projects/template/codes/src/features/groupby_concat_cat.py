...
from src.utils.dataframe import reduce_mem_usage
...
class GroupbyConcatCat(Feature):
    def create_features(
        ...
        with timer("load data"):
            ...
            len_train = len(train)
            test = test_df.copy()
            train_combi = pd.read_feather("./features/ConcatCategory_train.ftr")
            test_combi = pd.read_feather("./features/ConcatCategory_test.ftr")
            combi_cat_cols = test_combi.columns.tolist()

        with timer("concat combi"):
            train = pd.concat([train, train_combi], axis="columns")
            org_cols = train.columns.tolist()
            test = pd.concat([test, test_combi], axis="columns")

        with timer("concat train and test"):
            total = pd.concat([train, test], ignore_index=True).reset_index()
            del train, test
            gc.collect()
