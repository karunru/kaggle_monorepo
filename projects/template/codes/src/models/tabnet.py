...
class TabNet(BaseModel):
    ...
    def fit(
        ...
        **kwargs,
        ...
        for col in categorical_cols:
            ...
            categorical_dims[col] = len(set(x_train[col].values) | set(x_valid[col].values))

        cat_idxs = [i for i, f in enumerate(x_train.columns) if f in categorical_cols]
        cat_dims = [categorical_dims[f] for i, f in enumerate(x_train.columns) if f in categorical_cols]
    ...
    def predict(self, model: TabNetModel, features: AoD) -> np.ndarray:
        ...
        numerical_cols = [col for col in features.columns if col not in self.config["categorical_cols"]]
        for col in numerical_cols:
