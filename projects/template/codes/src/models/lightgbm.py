...
class LightGBM(BaseModel):
    ...
    def fit(
        ...
        **kwargs,
        ...
        if "early_stopping_rounds" in config["model"]["model_params"].keys():
            callbacks += [
                log_early_stopping(
                    stopping_rounds=config["model"]["model_params"]["early_stopping_rounds"],
    ...
    def get_feature_importance(self, model: LGBMModel) -> np.ndarray:
        return dict(
            zip(model.feature_name(), model.feature_importance(importance_type="gain"), strict=False),
        )
...
def log_early_stopping(stopping_rounds, first_metric_only=False):
        ...
        if env.iteration == env.end_iteration - 1:
            logging.debug(
                ...
                % (
                    best_iter[i] + 1,
                    "\t".join(
                        [lgb.callback._format_eval_result(x) for x in best_score_list[i]],
                    ),
                ),
            )
            if first_metric_only:
                logging.debug(f"Evaluated only: {eval_name_splitted[-1]}")
    ...
    def _callback(env):
        ...
        for i in range(len(env.evaluation_result_list)):
            ...
            if (
                env.evaluation_result_list[i][0] == "cv_agg" and eval_name_splitted[0] == "train"
